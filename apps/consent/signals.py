...
def merge_session_consent(sender: Any, user: Any, request, **kwargs) -> None:
    ...
    # 2️⃣ Retrieve active policy (model instance)
    ...
    try:
        with transaction.atomic():
            user_rec, created = ConsentRecord.objects.select_for_update().get_or_create(
                ...
                site_domain=site_domain,
                defaults={
                    "accepted_categories": session_rec.accepted_categories,  # type: ignore[attr-defined]
                    "session_key": session_key,
                },
            )
            ...
            if created:
                ...
                if user_rec.accepted_categories != session_rec.accepted_categories:  # type: ignore[attr-defined]
                    user_rec.accepted_categories = session_rec.accepted_categories  # type: ignore[attr-defined]
